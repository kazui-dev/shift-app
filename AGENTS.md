...
- 変更は機能の所有者に置く。Web の `app` と `routes` は機能を組み立ててよいが、汎用の `components`、`lib`、共有 package から機能固有コードを参照しない。境界入力は Web と API のそれぞれで検証し、認証・認可は必ず API で行う。
- 開発・検証・build の入口は `vp`、workspace と lockfile は pnpm。生成物の `apps/api/worker-configuration.d.ts` と `apps/web/src/routeTree.gen.ts` は直接編集しない。
