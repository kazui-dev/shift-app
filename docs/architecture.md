...
遅刻・欠勤・その取り消しは、勤怠通知botがシフトのルームへ非公開メッセージとして投稿する。本文は見出しと氏名・シフト・内容を1行ずつ、Pushは同じ内容を1行で送る（`domain/attendance-notice.ts`）。非公開メッセージは連絡した本人とシフトの見守り役だけが読め、Webはその旨を本文の下に表示する。責任者にはルームをミュートしていてもPushを送り、本人には送らない。判定と範囲は[データベース](database.md)の`ChatRoom` Durable Object SQLiteの項を参照。
