...
リンクプレビューは本文の最初のリンクだけを対象にする。ルーム閲覧権限とメッセージのリンクを確認してから、Workers Cacheを前に置いた`SharedCache` entrypointへURLを渡す。`SharedCache`は利用者を含まない経路とqueryをキーに、ページの説明を24時間（読めなかった場合は5分）、ページの画像を336×336に切り抜いた静止WebPを7日保持する。ブラウザへはどちらも`private, no-store`で返し、Webはカードの画像を一覧用の縮小画像と同じメモリと端末のキャッシュに置く。送信・編集時にカードの画像を先に作り、その過程で説明もキャッシュする。
