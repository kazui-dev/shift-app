...
通知設定のUIはトグルだけとし、許可状態の警告行・再要求ボタン・ヘルプは置かない。トグルは操作直後に切り替わり、設定を順番に保存する。保存失敗時だけ直前の確定値へ戻して「通知設定を保存できませんでした」と通知する。ON操作では必要ならその場で許可を要求し、拒否・未選択でもONを保持してトーストを出さない。ブラウザーの許可状態は購読登録のために内部で扱い、OS設定の即時反映は保証しない。許可済みかつONなら購読を登録するが、購読登録は設定操作を待たせない。操作に伴う登録の通信失敗は「通知の登録に失敗しました」と通知し、ONは維持する。OFFはサーバー配信だけを停止し、ブラウザー購読を解除しない。許可要求は起動・復帰・OFFでは行わない。設定変更・再起動を促すトーストやデバッグ用の表示は設けない。
