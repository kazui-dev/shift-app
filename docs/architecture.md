...
リンクプレビューは本文の最初のリンクだけを対象にし、送信・編集時にサーバーが取得してメッセージと一緒に保存する。取得は、Workers Cacheを前に置いた`SharedCache` entrypointを通す。`SharedCache`は利用者を含まない経路とqueryをキーに、ページの説明を24時間（読めなかった場合は5分）保持する。3秒以内に読めないリンクにはカードを付けない。履歴・送信の応答・イベントはプレビューを含むので、Webは描画の時点でカードの有無と大きさを決め、後からカードが出て履歴がずれることはない。カードの画像は、ルーム閲覧権限と保存されたプレビューを確認してから`SharedCache`へ渡し、336×336に切り抜いた静止WebPを7日保持する。ブラウザへは`private, no-store`で返し、Webはカードの画像を一覧用の縮小画像と同じメモリと端末のキャッシュに置く。送信・編集時にカードの画像も先に作る。
