...
Push購読toggleはアプリ起動時に端末状態を一度だけ読み込む共有storeを使い、設定画面の初回描画前に確定させる。操作時は同期中も入力を受け付け、表示を即座に最新の選択へ合わせる。外部I/Oは直列化して最後の選択へ収束させ、成功時は表示をそのまま確定して追加通知を出さない。最終状態の同期に失敗した場合だけ、storeが保持する直前の確定状態へ戻してエラーを通知する。それ以外のoptimistic updateは、操作ごとにrollback、server responseとの再同期、競合時の表示を定義してから導入する。出勤や遅刻欠勤など時間・状態に依存するmutationは、安全な競合仕様を決めるまでoffline queueへ入れない。
