...
- タイムライン上のシフトのカードに、出勤と遅刻・欠勤のボタンを置く。ボタンは開始30分前から終了までのカードにだけ出し、高さ80px以上のカードでは名前と時刻の下、それより低いカードでは名前の横に並べる。出勤後はボタンの代わりに出勤時刻を出す
- 出勤時に大学付近にいることを確認できない（位置情報が許可されていない、大学から離れている、正確に取得できない）ときは、理由と「出勤は記録され、あとで責任者が確認します。」を示して、このまま出勤するか確認する。キャンセルしたら何も記録しない
- 遅刻・欠勤の連絡はカレンダー上のシートで送る。遅刻か欠勤かを先に選び、遅刻では到着見込みを10分後・30分後・1時間後・時刻指定・未定から選ぶ。理由は任意
- 勤務時間内だけ押せるようにする
