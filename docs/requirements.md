...
オフラインで永続化するQueryは本人のタイムライン、参加中のチャットルーム、メッセージ履歴に限定する。管理画面、名簿、権限、チャット宛先候補は永続化しない。オフライン中はシフト確認と保存済みチャットの閲覧、冪等なチャット送信待ちだけを許可し、シフト希望、出勤、遅刻・欠勤連絡、管理操作、Push購読変更、チャットルーム作成は許可しない。
