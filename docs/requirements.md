...
- モバイルではボトムナビゲーションを使う
- Apple カレンダーのような、日付移動と時間軸が近い UI を参考にする
- 日、週、月はそれぞれ横スワイプで連続して移動でき、移動中の再操作も途切れず受け付ける
- 月表示のタップ領域を見た目と一致させ、タップすると日付pickerを開く
- PWA更新通知は本文を改行させない範囲で更新アイコン付きactionを表示し、実行中もactionの文言位置を動かさない
...
- 最後にオンラインで認証を確認した利用者による、保存済みデータの閲覧
- 閲覧可能なチャットの一覧用縮小画像とリンクカード画像の端末保存（最後の表示から14日・100MBまで）
- オフライン時のメッセージ送信待ち
- 安全な冪等性と競合規則を定義できたステータス更新の送信待ち

認証済み利用者のオフライン閲覧は、最後のオンライン確認から24時間以内に限定する。通信不能時だけ保存済みの認証状態へフォールバックし、サーバーが匿名状態または401/403を返した場合は利用者情報とQuery cacheを破棄する。利用者が変わった場合も旧利用者のQuery cacheと送信待ちを引き継がない。

オフラインで永続化するQueryは本人の割当、参加中のチャットルーム、メッセージ履歴に限定する。管理画面、名簿、権限、チャット宛先候補は永続化しない。オフライン中はシフト確認と保存済みチャットの閲覧、冪等なチャット送信待ちだけを許可し、シフト希望、出勤、遅刻・欠勤連絡、管理操作、Push購読変更、チャットルーム作成は許可しない。

オンライン復帰時は認証状態を再確認してから通常モードへ戻す。保存済み認証はAPI認可の代わりにせず、オンライン操作の認可は従来どおりWorker側で毎回確認する。
...
シフト管理者は年度ごとに希望を入力できる日付を設定する。利用者は設定済みの日付に対して、固定枠ではなく開始・終了を持つ複数の時間帯を提出する。各時間帯の開始と終了は同じ日本日付に属し、同一 member・年度の時間帯は重複不可とする。担当の割当は activity の範囲内かつ、同一 member の既存割当と重複しないことを必須とする。
...
Web Pushは利用者の明示操作によるtoggleで端末ごとに有効化する。端末の購読状態はアプリ起動時に先行取得し、設定画面は最初から確定した状態を表示する。toggleは同期中も操作でき、最後に選んだ状態を即時表示する。未確定の選択は認証利用者単位で同じtabの再読み込みを短時間だけまたいで復元し、登録または解除を再開する。登録と解除は直列に処理し、成功時は追加のtoastを表示しない。最新の選択の同期に失敗した場合だけ直前の確定状態へ戻し、エラーを通知する。初期版は新規割当時と開始約10分前に通知し、同一assignment・端末・通知種別の二重配送を防ぐ。割当時刻の編集APIと変更通知は未実装で、編集履歴と通知のevent IDを合わせて追加する。通知許可がなくてもカレンダーなどの本体機能は利用できる。
